...
class Handwriting {
  ...
  captureDrawing() {
    let group = new fabric.Group(this.canvas.getObjects()),
      ...
      image = this.canvas.contextContainer.getImageData(
        left * scale,
        top * scale,
        width * scale,
        height * scale
   ...
   */
  bindEvents() {
    this.outputEl.placeholder = "Draw and Predict ......";
    ...
    this.canvas.freeDrawingBrush.width =
      parseInt(this.drawingLineWidthEl.value, 10) || 1;
    this.drawingLineWidthEl.previousSibling.innerHTML =
      this.canvas.freeDrawingBrush.width;
